...
### 🆕 Phase 2 계획 (Auto-Generation) - 2025년 10월

**완전 자동화 시스템 구축**

#### 주요 목표
- 🤖 **100% 자동화**: Microsoft 블로그 자동 모니터링 및 카드뉴스 생성
- 💾 **DB 통합**: 모든 프로젝트 Firestore에 영구 저장
- 📧 **알림 시스템**: 생성 완료 시 이메일 발송
- 🔄 **워크플로우**: 크롤링 → 요약 → 생성 → 저장 → 알림

#### 타겟 사이트 (Microsoft)
1. https://blogs.microsoft.com/
2. https://news.microsoft.com/source/asia/region/korea/?lang=ko
3. https://www.microsoft.com/en-us/security

#### 핵심 기능
1. **크롤링 시스템**: RSS + Playwright
2. **자동 생성 파이프라인**: 사용자 개입 없이 완전 자동
3. **프로젝트 관리**: 저장, 수정, 버전 관리
4. **메인 대시보드**: 3개 메뉴 (크롤링 설정 / Phase 1 / 프로젝트)

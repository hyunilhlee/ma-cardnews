...
### 🆕 Phase 2 계획 (RSS Auto-Generation) - 2025년 10월

**RSS 기반 자동화 시스템 구축 (단순화 및 개발 기간 단축)**

#### 주요 목표
- 📡 **RSS 전용**: RSS 피드가 있는 사이트만 지원 (빠른 개발)
- 💾 **DB 통합**: 모든 프로젝트 Firestore에 영구 저장
- 📧 **사용자별 알림**: 대시보드에서 이메일 등록/관리
- 🔄 **워크플로우**: RSS 파싱 → 요약 → 생성 → 저장 → 알림
- ⚡ **개발 기간**: 3-4주 (기존 4-6주에서 단축)
...
#### 핵심 기능
1. **RSS 크롤링 시스템**: `feedparser` 사용 (간단하고 빠름)
2. **자동 생성 파이프라인**: 사용자 개입 없이 완전 자동
3. **프로젝트 관리**: 저장, 수정, 버전 관리
4. **이메일 알림 관리**: 사용자가 직접 이메일 등록/관리
5. **메인 대시보드**: 4개 메뉴 (RSS 설정 / Phase 1 / 프로젝트 / 알림)

**상세 개발 계획**: [PHASE2_SPEC.md](../PHASE2_SPEC.md) 참조

---

### 📌 Phase 3 계획 (Web Scraping & Design) - 미래

**RSS 없는 사이트 지원 및 디자인 기능 추가**

#### 추가 기능
1. **웹 스크래핑**: Playwright로 RSS 없는 사이트 크롤링
2. **디자인 시스템**: 템플릿, 색상, 폰트 커스터마이징
3. **이미지 통합**: AI 이미지 생성 및 편집
4. **내보내기**: PNG, PDF 형식 다운로드
